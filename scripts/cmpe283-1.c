...
// * See SDM volume 3, section 24.6.1
// */
//struct capability_info vmx_entry[5] =
//{
//  { 0, "External Interrupt Exiting" },
//  { 3, "NMI Exiting" },
//  { 5, "Virtual NMIs" },
...
// * See SDM volume 3, section 24.6.1
// */
//struct capability_info vmx_exit[5] =
//{
//  { 0, "External Interrupt Exiting" },
//  { 3, "NMI Exiting" },
//  { 5, "Virtual NMIs" },
...
/*
 * Entry capabilities
 * See SDM volume 3, section 24.6.4
 */
...
/*
 * Exit capabilities
 * See SDM volume 3, section 24.6.5
 */
...
detect_vmx_features(void)
{
  ...
  pr_info("Probased Controls2 MSR: 0x%llx\n",
    (uint64_t)(lo | (uint64_t)hi << 32));
  ...
  //report_capability(vmx_entry, 5, lo, hi);

  /* Exit controls */
  rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
  pr_info("Exit Controls MSR: 0x%llx\n",
    (uint64_t)(lo | (uint64_t)hi << 32));
  report_capability(vmx_exit, 14, lo, hi);
  //report_capability(vmx_exit, 5, lo, hi);
}
